...
class RelysiaManager {
    ...
    static Future<Map> authUser(String email, String password) async
    {
      Map responseData = {};
      try{
        String bodyCode =  jsonEncode(
                  <String, String>{'email': email, 'password': password});
        await http
            .post(
              Uri.parse(apiUrlAuth),
              headers: <String, String>{
                HttpHeaders.authorizationHeader: jsonEncode(<String, String>{
                  'Key': serviceId,
                  'Value': 'true'
                }),
    ...
    static Future<int> createWallet(String token) async
    {
      ...
      try{
        await http.get(
            Uri.parse('https://api.relysia.com/v1/createWallet?serviceID=$serviceId&walletTitle=00000000-0000-0000-0000-000000000000&paymailActivate=true'),
    ...
    static Future<int> payNow(String token,String payMail, String amount) async
    {
      ...
      try{
        await http
          .post(
            Uri.parse('https://api.relysia.com/v1/send?serviceID=9ab1b61e-92ae-4612-9a4f-c5a102a6c068&authToken=$token'),
            headers:{
                HttpHeaders.authorizationHeader: jsonEncode(<String, String>{
                  'Key': serviceId,
                  'Value': 'true'
            ...
            body: '{ "dataArray" : [{"to" : "$payMail","amount" : $amount ,"tokenId" : "$token_id"}]}', 
          )
          .then((res) => {
            respondData = jsonDecode(res.body),

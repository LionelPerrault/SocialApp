...
// ignore: must_be_immutable
class TwoFactorAuthenticationScreenState extends mvc.StateMVC<TwoFactorAuthenticationScreen> {
            ...
            : Row(
              ...
              children: [
                PrettyQr(
                    typeNumber: 10,
                    size: 130,
                    data: 'otpauth://totp/shnatter?Algorithm=SHA1&digits=6&secret=JBSWY3DPEHPK3PXP&issuer=shnatter&period=30',
                    ...
                    children: [
                      ...
                      Row(
                        ...
                        children: [
                          IconButton(
                            onPressed: () {
                              FlutterClipboard.copy('JBSWY3DPEHPK3PXP').then(( value ) =>
                              Helper.showToast('Secret Key was cofied!'));
                            },
                            iconSize: 20,
                            icon: const Icon(
                              Icons.copy,
                      ...
                      onPressed: () async {
                        if(con.isEnableTwoFactor){
                          if(verificationCode.length == 6){
                            checkVerificationCode = await con.enableDisableTwoFactorAuthentication(verificationCode, 'disable');
                          }
                          if(checkVerificationCode){
                            Helper.showToast('Two-factor Authentication was disabled!');
                            verificationCode = '';
                          }else{
                            Helper.showToast('Verification code is incorrect!');
                          }
                        }else{
                          if(verificationCode.length == 6){
                            checkVerificationCode = await con.enableDisableTwoFactorAuthentication(verificationCode, 'enable');
                          }
                          if(checkVerificationCode){
                            Helper.showToast('Two-factor Authentication was enabled!');
                            verificationCode = '';
                          }else{
                            Helper.showToast('Verification code is incorrect!');
                          }
